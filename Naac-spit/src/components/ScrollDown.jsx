...
//       <div className={`font-extrabold mt-1 ml-2 text-xl `}>{customContent}</div>
...
function ScrollDown({ customContent, isOpen, openBar, children }) {
  return (
    ...
    >
      <div className='flex justify-between overflow-hidden'>
        <div style={{ fontSize: '1em', margin: '2px' }}>{customContent}</div>
        <div className=''>
          {isOpen ? (
            <FaMinus
              onClick={openBar}
      ...
      {isOpen && <div className='m-3'>{children}</div>}
    </motion.div>
  );
}
export default ScrollDown;
